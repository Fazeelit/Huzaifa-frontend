...
const PRELOAD_RESOURCE_RULES = [
  {
    endpoint: "/customers",
    canLoad: (permissions) => hasModuleAccess("CUSTOMER", permissions),
  },
  {
    endpoint: "/suppliers",
  ...
  {
    endpoint: "/purchases",
    canLoad: (permissions) => hasModuleAccess("PURCHASE", permissions),
  },
  {
    endpoint: "/expenses",
    canLoad: (permissions) => hasModuleAccess("EXPENSE", permissions),
  },
  {
    endpoint: "/sales",
    canLoad: (permissions) => hasModuleAccess("SALE", permissions),
  },
  {
    endpoint: "/roles",
    canLoad: (permissions) => hasModuleAccess("ROLE", permissions),
  },
  {
    endpoint: "/user-management",
    canLoad: (permissions) => hasModuleAccess("USER", permissions),
  },
];
...
function clearInvalidAuthState() {
  ...
  if (typeof document !== "undefined") {
    document.cookie = "app_auth=; max-age=0; path=/; SameSite=Lax";
    document.cookie = "app_role=; max-age=0; path=/; SameSite=Lax";
  }
}
...
function resolveResourceConfig(endpoint) {
  return (
    CRUD_RESOURCE_CONFIGS.find((config) => config.match(endpoint)) || null
  );
}
...
function inferTargetId(endpoint) {
  ...
  for (const pattern of patterns) {
    ...
    if (
      candidate &&
      !["create", "createCustomer", "createSupplier", "createProduct", "createPurchase", "createExpense", "createRole", "createUser"].includes(candidate)
    ) {
      return candidate;
    }
  }
...
function findCachedEntityFromStore(store, config, targetId) {
  ...
  for (const [endpoint, cachedResponse] of Object.entries(store || {})) {
    if (!config.match(endpoint)) continue;
...
function buildOptimisticPaymentEntity(baseEntity, payload) {
  ...
  const previousPaid = Number(
    baseEntity?.paidAmount ?? baseEntity?.cashReceived ?? 0,
  );
...
function applyOperationToStore(store, operation, serverResponse = null) {
  ...
  if (operation.action === "create") {
    ...
    nextList = nextList.filter((entry) => {
      const entryId = getEntityId(entry);
      return !idsMatch(entryId, targetId);
    });
  }
  ...
  if (
    ...
    config.detailMatchers?.length &&
    (targetId || getEntityId(optimisticEntity))
  ) {
    ...
    nextStore[detailEndpoint] = buildDefaultDetailResponse(
      config,
      optimisticEntity,
    );
  }
...
async function authRequest(
  ...
  try {
    ...
    if (includeAuth && isAuthenticationError(error)) {
...
export async function apiRequest(endpoint, options = {}) {
  ...
  if (normalizedMethod === "GET") {
    try {
      ...
      if (config) {
        const detailId = extractDetailId(config, endpoint);
        const cachedEntity = findCachedEntityFromStore(
          cachedResponses,
          config,
          detailId,
        );
  ...
  for (const operation of queue) {
    try {
      ...
      if (isRecoverableSyncError(error)) {
        remainingQueue.push(
          operation,
          ...queue.slice(queue.indexOf(operation) + 1),
        );
        break;
      }
...
export function getStoredAuthToken() {
  ...
  const rawToken = normalizeStoredValue(storage.getItem(AUTH_TOKEN_KEY));
