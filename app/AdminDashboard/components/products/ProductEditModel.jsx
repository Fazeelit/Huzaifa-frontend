...
const fieldClass =
  "min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-base leading-8 text-slate-700 shadow-sm outline-none transition focus:border-sky-300 focus:ring-2 focus:ring-sky-100";
...
const ProductEditModal = ({ productId, onClose }) => {
  ...
  const [codeSeed, setCodeSeed] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        setFetching(true);
        const res = await apiRequest(`/products/getProductById/${productId}`, {
          method: "GET",
        });
      ...
      <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-slate-900/50 p-3 backdrop-blur-sm sm:p-4">
        <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-white/80 bg-white shadow-2xl">
          <div className="flex items-center justify-between gap-3 rounded-t-2xl bg-gradient-to-r from-sky-600 to-cyan-600 p-4 text-white sm:p-6">
            <h2 className="min-w-0 text-xl font-bold sm:text-2xl">Edit Product Master Data</h2>
                  ...
                  <div key={key}>
                    ...
                    {key === "category" ? (
                      ...
                      <input
                        ...
                        className={`${key === "code" ? `${fieldClass} bg-slate-100 text-slate-500` : fieldClass} ${
                    ...
                    {key === "code" ? (
                      <p className="mt-1 text-xs text-slate-500">
                        Auto generated from category and product name.
                      </p>
                    ) : null}
                  </div>
                ))}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`${fieldClass} ${
                      formData.discountAllowed ? "" : "cursor-not-allowed bg-slate-100 text-slate-400"
                    }`}
                    style={urduInputStyle}
                    placeholder="0"
                  />
